...
const SimpleAgent_1 = require("../services/SimpleAgent");
class CrawlerChatViewProvider {
    ...
    constructor(_extensionUri) {
        ...
        this.crawlerService = new CrawlerService_1.CrawlerService(crawlerConfig);
        this.intelligentAgent = new SimpleAgent_1.SimpleIntelligentAgent();
        // 智能代理欢迎消息
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.apiResults.length > 0) {
                    ...
                    result.apiResults.slice(0, 10).forEach((api, index) => {
     ...
     * 处理简单URL输入（兼容性）
     */
    async handleSimpleUrlInput(url) {
    ...
    async showInfo() {
        ...
        vscode.window.showInformationMessage('JS爬虫分析器 v3.1.0 - Playwright单引擎版\n\n' +
            '🚀 主要功能：\n' +
...
- 无需手动操作，一切都是自动的！`, false);
    ...
    async analyzeUploadedFile() {
        try {
            ...
            // 让用户选择文件
            const fileOptions = listResult.files.map((file) => ({
