...
3. 防火墙阻止了连接`);
            ...
            // 连接到现有浏览器
            this.browser = await playwright_1.chromium.connectOverCDP(wsEndpoint);
            if (this.config.verbose) {
     ...
     * 启动用户的本地浏览器并添加调试参数
     */
    async launchUserBrowserWithDebug(debugPort) {
        ...
        try {
            ...
            if (this.config.useRealUserData) {
                // 使用真实数据时的温和参数
                args.push('--disable-blink-features=AutomationControlled', '--disable-web-security', '--no-sandbox', '--disable-dev-shm-usage');
                if (this.config.verbose) {
                ...
                args.push(
                // 增强反检测参数
                '--disable-blink-features=AutomationControlled', '--disable-features=VizDisplayCompositor', '--disable-ipc-flooding-protection', '--disable-web-security', '--disable-features=TranslateUI', '--disable-extensions-except', '--disable-extensions', '--disable-component-extensions-with-background-pages', '--no-default-browser-check', '--no-pings', '--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage', '--disable-accelerated-2d-canvas', '--disable-gpu-sandbox', '--disable-background-networking', '--disable-sync', '--disable-prompt-on-repost', '--disable-client-side-phishing-detection', '--disable-component-update', '--disable-domain-reliability', '--disable-features=AudioServiceOutOfProcess', '--disable-features=ImprovedCookieControls', '--disable-features=LazyFrameLoading', '--disable-features=GlobalMediaControls', '--disable-hang-monitor', '--disable-plugins-discovery', '--disable-print-preview', '--disable-notifications', '--mute-audio', 
                // 隐蔽性参数
                '--incognito', '--disable-logging', '--silent-debugger-extension-api', '--autoplay-policy=user-gesture-required', '--disable-restore-session-state', '--disable-ipc-flooding-protection', 
                // 性能优化
                '--max_old_space_size=4096', '--memory-pressure-off', '--disable-background-networking');
            }
            // 启动浏览器进程
     ...
     * 检测用户真实的浏览器数据目录
     */
    async detectUserDataDirectories() {
        const fs = require('fs');
            ...
            try {
                ...
                HTMLCanvasElement.prototype.getContext = function (contextType, ...args) {
                    ...
                    if ((contextType === 'webgl' || contextType === 'webgl2') && context) {
                        ...
                        webglContext.getParameter = function (parameter) {
                            if (parameter === 37445)
                                return 'Intel Inc.';
                            if (parameter === 37446)
     ...
     * 获取浏览器的WebSocket调试端点
     */
    async getBrowserWebSocketEndpoint(port) {
        try {
            ...
            if (this.config.verbose) {
                console.log(`无法从端口${port}获取WebSocket端点:`, error.message);
            }
            return null;
        }
    }
